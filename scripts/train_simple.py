...
def extract_features(df: pd.DataFrame) -> np.ndarray:
    ...
    if 'timestamp' in df.columns:
        ...
        features.append((timestamps.dt.hour < 6).astype(int).values | 
                       (timestamps.dt.hour > 22).astype(int).values)  # is_night
    else:
    ...
    turkish_cities = {'İstanbul', 'Istanbul', 'Ankara', 'İzmir', 'Izmir', 'Bursa', 
                      'Antalya', 'Adana', 'Konya', 'Gaziantep', 'Mersin', 'Diyarbakır',
    ...
    suspicious_keywords = ['acil', 'bitcoin', 'kripto', 'casino', 'bahis', 
                          'offshore', 'anonim', 'kumar', 'usdt', 'gizli']
    ...
    features.append((df['amount'] > MASAK_THRESHOLD * 0.9).astype(int).values & 
                   (df['amount'] < MASAK_THRESHOLD).astype(int).values)  # structuring
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        z_score = np.where(sender_std > 0, 
                          (df['amount'].values - sender_mean) / sender_std, 
                          0)
...
def evaluate_model(name: str, y_true: np.ndarray, y_scores: np.ndarray) -> dict:
    ...
    log(f"{name}: Acc={metrics['accuracy']:.4f} P={metrics['precision']:.4f} "
