...
def generate_blacklist_keywords() -> list[dict]:
    ...
    return [
        generate_transaction(description=f"Ödeme için - {kw}") for kw in random.sample(keywords, 3)
    ]


def generate_high_amount() -> list[dict]:
    """Generate unusually high amount transactions."""
    return [generate_transaction(amount=random.uniform(200000, 1000000)) for _ in range(3)]
...
def main():
    ...
    # Generate scenario
        ...
        console.print(
            f"[green]Generated {len(transactions)} transactions for '{args.scenario}' scenario[/green]"
        )
