        ...
        for pep_name, pep_info in DEMO_PEP_DATABASE.items():
            ...
            if match_score >= 70:
                match_type = (
                    MatchType.EXACT
                    if match_score >= 95
                    else MatchType.PARTIAL if match_score >= 85 else MatchType.FUZZY
                )
                ...
                matches.append(
                    ScreeningMatch(
                        ...
                        details={
                            "pep_level": (
                                pep_info.get("level", PEPLevel.DOMESTIC_MEDIUM).value
                                if isinstance(pep_info.get("level"), PEPLevel)
                                else str(pep_info.get("level", ""))
                            ),
                    ...
                    matches.append(
                        ScreeningMatch(
                            ...
                            details={
                                "list": (
                                    sanction_list.value
                                    if isinstance(sanction_list, SanctionsList)
                                    else str(sanction_list)
                                ),
                                "reason": sanction_info.get("reason", ""),
                                "added_date": sanction_info.get("added_date", ""),
                                "country": sanction_info.get("country", ""),
                                "matched_alias": (
                                    check_name if check_name != sanctioned_name else None
                                ),
                            },
                        )
                    )
