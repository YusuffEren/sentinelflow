...
class KafkaSettings(BaseSettings):
    ...
    topic_transactions: str = Field(
        default="transactions", description="Topic for incoming transactions"
    )
    topic_alerts: str = Field(default="alerts", description="Topic for fraud alerts")
    consumer_group: str = Field(default="sentinelflow-consumers", description="Consumer group ID")
...
class GeneratorSettings(BaseSettings):
    ...
    fraud_ratio: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Ratio of fraudulent transactions"
    )
