...
class Evidence(ContractBase):
    ...
    detector_type: DetectorType = Field(..., description="Detection engine that produced this evidence")
    detector_version: str = Field(default="1.0.0")
    
    # What was detected
    rule_id: str | None = Field(default=None, description="Rule ID if rule-based")
    pattern_name: str | None = Field(default=None, description="Pattern name (e.g., 'circular_ring_3_hop')")
    
    # Scores
    confidence: float = Field(default=0.0, ge=0.0, le=1.0, description="Detection confidence")
    contribution: float = Field(default=0.0, ge=0.0, le=1.0, description="Contribution to final score")
