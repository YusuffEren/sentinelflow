...
class AlertRepository:
    ...
    def create(
        ...
        *,
        ...
        if alert_data.evidence:
            evidence_data = [e.model_dump(mode="json") if hasattr(e, "model_dump") else e for e in alert_data.evidence]
        
        model = AlertModel(
            alert_id=alert_id,
            fraud_type=alert_data.fraud_type.value if isinstance(alert_data.fraud_type, FraudType) else alert_data.fraud_type,
            severity=alert_data.severity.value if isinstance(alert_data.severity, Severity) else alert_data.severity,
    ...
    async def create_async(
        ...
        *,
        ...
        existing = await self._check_duplicate_async(alert_data.transaction_id, alert_data.fraud_type)
        if existing:
            logger.debug(f"Duplicate alert skipped: {existing.alert_id}")
            return Alert.model_validate(existing.to_dict())
    ...
    def get_stats(
        ...
        total = self._session.execute(
            select(func.count(AlertModel.alert_id)).where(and_(*conditions)) if conditions else select(func.count(AlertModel.alert_id))
        ).scalar() or 0
        
        # By fraud type
        by_type_query = (
            select(AlertModel.fraud_type, func.count(AlertModel.alert_id))
            .group_by(AlertModel.fraud_type)
        )
        if conditions:
        ...
        # By severity
        by_severity_query = (
            select(AlertModel.severity, func.count(AlertModel.alert_id))
    ...
    def _check_duplicate_sync(self, transaction_id: str, fraud_type: FraudType | str) -> AlertModel | None:
    ...
    async def _check_duplicate_async(self, transaction_id: str, fraud_type: FraudType | str) -> AlertModel | None:
