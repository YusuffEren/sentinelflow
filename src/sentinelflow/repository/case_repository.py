...
class CaseRepository:
    ...
    def create(self, case_data: CaseCreate | dict[str, Any]) -> Case:
        ...
        model = CaseModel(
            ...
            status="new",
            priority=(
                case_data.priority.value
                if isinstance(case_data.priority, CasePriority)
                else case_data.priority
            ),
            primary_fraud_type=(
                case_data.primary_fraud_type.value if case_data.primary_fraud_type else None
            ),
    ...
    def update_status(
        ...
        stmt = update(CaseModel).where(CaseModel.case_id == case_id).values(**values)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        by_status = dict(
            self._session.execute(
                select(CaseModel.status, func.count(CaseModel.case_id)).group_by(CaseModel.status)
            ).all()
        )
        ...
        by_priority = dict(
            self._session.execute(
                select(CaseModel.priority, func.count(CaseModel.case_id)).group_by(
                    CaseModel.priority
                )
            ).all()
        )
        ...
        open_count = (
            self._session.execute(
                select(func.count(CaseModel.case_id)).where(CaseModel.status.in_(open_statuses))
            ).scalar()
            or 0
        )
