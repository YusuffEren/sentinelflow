...
class ABTestManager:
    ...
    def log_outcome(
        ...
        with self._lock:
            ...
            # Update bandit state
            if (
                test.split_strategy == SplitStrategy.MULTI_ARMED_BANDIT
                and test.test_id in self._bandit_state
            ):
    ...
    def get_results(self, test_id: str) -> ABTestResult:
        ...
        if (
            control.total_samples < test.min_sample_size
            or treatment.total_samples < test.min_sample_size
        ):
            ...
            result.samples_needed = test.min_sample_size - min(
                control.total_samples, treatment.total_samples
            )
            return result
