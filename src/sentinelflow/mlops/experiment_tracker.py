...
class ExperimentTracker:
    ...
    def start_run(
        ...
        with self._lock:
            ...
            # Generate run ID
            run_id = run_name or f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6]}"
    ...
    def list_runs(
        ...
        runs = [
            self._runs[rid] for rid in experiment.run_ids
    ...
    def compare_runs(
        ...
        for param in all_params:
            values = [
                self._runs[rid].params.get(param)
                for rid in run_ids
                if rid in self._runs
            ]
        ...
        for metric in target_metrics:
            values = [
                self._runs[rid].metrics.get(metric)
                for rid in run_ids
                if rid in self._runs
            ]
            comparison["metrics"][metric] = {
                ...
                "mean": np.mean([v for v in values if v is not None]) if any(v is not None for v in values) else None,
            }
