...
class FraudExplainer:
    ...
    def _explain_with_shap(
        ...
        try:
            ...
            if hasattr(model, 'predict_proba'):
                predict_fn = lambda x: model.predict_proba(x) if len(x.shape) == 2 else model.predict_proba(x.reshape(1, -1))
            ...
            for name, value in sorted_features[:self._top_n]:
                ...
                feature_val = features[self._feature_names.index(name)] if name in self._feature_names else 0
                explanation.top_reasons.append(
                    f"{desc}: {feature_val:.2f} (risk {direction})"
                )
    ...
    def _explain_with_features(
        ...
        if abs(amount_zscore) > 2.0:
            risk_scores.append((
                "amount_zscore", abs(amount_zscore),
        ...
        if tx_count_1h > 5:
            risk_scores.append((
                "sender_tx_count_1h", tx_count_1h,
        ...
        if features_dict.get("is_night", 0) > 0:
            risk_scores.append((
                "is_night", 2.0,
                "Gece saatlerinde yapılan işlem"
            ))
        ...
        if features_dict.get("is_international", 0) > 0:
            risk_scores.append((
                ...
                "Uluslararası transfer tespit edildi"
            ))
        ...
        if distance > 500:
            risk_scores.append((
                "city_distance_km", distance / 200,
                f"Şehirler arası mesafe: {distance:.0f} km"
            ))
        ...
        if percentile > 0.95:
            risk_scores.append((
                "amount_percentile", percentile * 5,
