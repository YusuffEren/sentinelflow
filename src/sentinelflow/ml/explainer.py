...
class FraudExplainer:
    ...
    def __init__(
        ...
        if feature_names is not None and not isinstance(feature_names, list):
            # Backward compatibility: older callers pass model as first arg.
            self._default_model = feature_names
            self._feature_names = []
        else:
    ...
    def explain(
        ...
        """
        # Backward compatibility: some callers pass prediction score as 2nd arg.
        ...
        if self._use_shap and active_model is not None and self._background_data is not None:
            explanation = self._explain_with_shap(features, active_model)
        else:
