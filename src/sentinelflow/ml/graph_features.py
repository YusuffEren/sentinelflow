...
class GraphFeatureEngine:
    ...
    def extract_vector(self, iban: str) -> np.ndarray:
        ...
        return np.array(
            [features.get(name, 0.0) for name in GRAPH_FEATURE_NAMES],
            dtype=np.float64
        )
    ...
    def _get_centrality_features(
        ...
        features["betweenness_centrality"] = float(bc_result["betweenness_approx"] or 0) if bc_result else 0.0
    ...
    def _get_community_features(
        ...
        if result:
            ...
            features["community_fraud_ratio"] = fraud_count / community_size if community_size > 0 else 0.0
        else:
        ...
        if inter_result:
            ...
            features["inter_community_ratio"] = inter_result["inter"] / total_edges if total_edges > 0 else 0.0
        else:
    ...
    def _get_path_features(
        ...
        try:
            ...
            features["avg_path_length"] = float(avg_result["avg_path_length"] or 3.0) if avg_result else 3.0
        except Exception:
        ...
        try:
            ...
            features["clustering_coefficient"] = float(cc_result["clustering_coef"] or 0) if cc_result else 0.0
        except Exception:
    ...
    def _get_temporal_graph_features(
        ...
        if burst_result:
            features["temporal_burst_score"] = min(float(burst_result["burst_ratio"] or 0), 10.0) / 10.0
        else:
    ...
    def _get_risk_propagation_features(
        ...
        features["contamination_score"] = min(float(cont_result["contaminated_targets"] or 0) / 10.0, 1.0) if cont_result else 0.0
...
class InMemoryGraphFeatureEngine:
    ...
    def extract_features(self, iban: str) -> Dict[str, float]:
        ...
        try:
            ...
            features["neighbor_fraud_ratio"] = fraud_neighbors / len(neighbors) if neighbors else 0.0
        except Exception:
            pass
