...
class AdvancedFeatureEngine:
    ...
    def extract(
        ...
        network_features = self._extract_network_features(
            receiver_iban, timestamp, activity
        )
        ...
        risk_features = self._extract_risk_features(
            amount, timestamp, receiver_iban, activity
        )
        ...
        self._update_activity(
            activity, amount, timestamp, receiver_iban, channel, device_id
        )
    ...
    def extract_vector(self, tx_data: Dict[str, Any]) -> np.ndarray:
        ...
        return np.array(
            [features.get(name, 0.0) for name in ADVANCED_FEATURE_NAMES],
            dtype=np.float64
        )
    ...
    def _extract_behavioral_features(
        ...
        if len(activity.timestamps) >= self._min_samples:
            ...
            if len(activity.timestamps) >= 2:
                time_span = (max(activity.timestamps) - min(activity.timestamps)).total_seconds() / 3600
        ...
        features["receiver_novelty_score"] = 0.0 if receiver_iban in activity.receiver_counts else 1.0
        
        # Channel deviation
        typical_channels = set(activity.channels)
        features["channel_deviation_score"] = 0.0 if channel in typical_channels or len(typical_channels) == 0 else 1.0
    ...
    def _extract_benford_features(self, amount: float) -> Dict[str, float]:
        ...
        if len(self._global_first_digits) >= 100:
            ...
            if total > 0:
                ...
                chi_sq = np.sum((observed_dist - BENFORD_EXPECTED) ** 2 / (BENFORD_EXPECTED + 1e-10))
    ...
    def _extract_temporal_features(
        ...
        features["tx_velocity_7d"] = float(sum(1 for t in activity.timestamps if t >= seven_days_ago))
        
        # Burst detection (sudden spike in activity)
        if len(activity.timestamps) >= 10:
            # Compare last 1h to average hourly rate
            time_span_hours = (max(activity.timestamps) - min(activity.timestamps)).total_seconds() / 3600
        ...
        is_off_hours = local_hour < TURKISH_BANKING_HOURS[0] or local_hour >= TURKISH_BANKING_HOURS[1]
    ...
    def _extract_risk_features(
        ...
        one_day_ago = timestamp - timedelta(hours=24)
        recent_amounts = [a for t, a in zip(activity.timestamps, activity.amounts) if t >= one_day_ago]
        
        near_threshold_count = sum(
            1 for a in recent_amounts 
            if MASAK_THRESHOLD_TL * 0.8 <= a < MASAK_THRESHOLD_TL
        )
        ...
        if len(activity.timestamps) >= 5:
            ...
            unique_ratio = len(set(activity.receivers)) / len(activity.receivers) if activity.receivers else 0
    ...
    def _extract_statistical_features(
        ...
        seven_days_ago = timestamp - timedelta(days=7)
        recent_amounts = [a for t, a in zip(activity.timestamps, activity.amounts) if t >= seven_days_ago]
        
        if len(recent_amounts) >= 3:
            features["amount_volatility_7d"] = np.std(recent_amounts) / (np.mean(recent_amounts) + 1e-6)
        else:
