...
TURKISH_CITIES = [
    ...
    "Eskişehir", "Trabzon", "Samsun", "Denizli",
]
...
SUSPICIOUS_DESCRIPTIONS = [
    "bitcoin satış acil", "kripto transfer hemen", "casino ödemesi",
    "bahis kazancı transfer", "offshore hesap aktarım", "anonim transfer",
    "kumar borcu ödeme", "usdt binance çekim", "acil nakit gönder",
    "gizli transfer yapılacak", "poker masası ödemesi", "slot makinesi",
]
...
def _generate_name() -> str:
    ...
    first_names = ["Ahmet", "Mehmet", "Ali", "Fatma", "Ayşe", "Zeynep", "Emre", "Burak", "Selin", "Merve"]
...
class FraudDatasetLoader:
    ...
    def load_csv(
        ...
        transactions = self._convert_csv_to_transactions(
            df, amount_col, label_col, timestamp_col
        )
        ...
        logger.info(
            f"Features extracted: {feature_matrix.shape}, "
            f"fraud rate: {labels.mean()*100:.1f}%"
        )
    ...
    def _generate_normal_transaction(self, idx: int) -> dict[str, Any]:
        ...
        hour = self._rng.choices(range(24), weights=[
            1, 1, 1, 1, 1, 1, 2,  # 0-6
            5, 8, 10, 10, 10,       # 7-11
            10, 10, 10, 8, 8,       # 12-16
            6, 5, 4, 3, 2, 1, 1,   # 17-23
        ...
        base_time = datetime(2026, 1, 1) + timedelta(days=day_offset, hours=hour, minutes=self._rng.randint(0, 59))
    ...
    def _generate_night_fraud(self, idx: int) -> dict[str, Any]:
        ...
        day_offset = self._rng.randint(0, 30)
        tx["timestamp"] = (datetime(2026, 1, 1) + timedelta(days=day_offset, hours=hour, minutes=self._rng.randint(0, 59))).isoformat()
