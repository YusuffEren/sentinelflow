        ...
        def __init__(
            ...
            for i in range(num_layers):
                ...
                self.convs.append(GATConv(
                    in_ch, out_ch, heads=heads, concat=concat, dropout=dropout
                ))
...
class GNNFraudModel:
    ...
    def build_graph_from_neo4j(self, graph_engine: Any) -> None:
        ...
        edge_attr_np = np.array(edge_attrs, dtype=np.float32) if edge_attrs else np.zeros((0, 2), dtype=np.float32)
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            embeddings, predictions = self._network(
                self._torch_data.x, 
                self._torch_data.edge_index
            )
            ...
            self._network.eval()
            with torch.no_grad():
                _, val_predictions = self._network(
                    self._torch_data.x,
                    self._torch_data.edge_index
                )
